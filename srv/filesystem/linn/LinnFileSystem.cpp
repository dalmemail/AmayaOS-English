...
int main(int argc, char **argv)
{
    ...
    {
	BootModule *bm = new BootModule("/boot/eng/boot.linn.gz");
    ...
    : FileSystem(p), storage(s), groups(ZERO)
{
    ...
    if ((e = s->read(LINN_SUPER_OFFSET, &super,
		     sizeof(super))) <= 0)
    {
	syslog(LOG_ERR, "error al leer superblocks: %s",
	       strerror(e));
	exit(EXIT_FAILURE);
    }
    /* Verify magic. */
	...
	{
	    syslog(LOG_ERR, "error de lectura del group descriptor: %s",
		   strerror(e));
	    exit(EXIT_FAILURE);
	}
    ...
    /* Done. */
    syslog(LOG_INFO, "montado como '%s'", p);
}
...
LinnInode * LinnFileSystem::getInode(u32 inodeNum)
{
    ...
    {
        syslog(LOG_ERR, "error de lectura del inode: %s",
	       strerror(e));
	return ZERO;
    }
